...
namespace Repositorio
{
    public class RepositorioFuncionarios
    {
        ...
        {
            return Funcionarios.Where(nomeFunc => nomeFunc.Nome.Contains(nome)).ToList();
        }        
        ...
        public IList<Funcionario> FiltrarPorIdadeAproximada(int idade)
        {
            ...
            return this.Funcionarios.FindAll(func =>
                dataAtual - func.DataNascimento.Year > idade &&
                dataAtual - func.DataNascimento.Year <= idade + 5 ||
                dataAtual - func.DataNascimento.Year < idade &&
                dataAtual - func.DataNascimento.Year >= idade - 5
            ).ToList() ;
        ...
        {
            var retornoBuscaRapida = this.Funcionarios.Select(x => new { NomeFuncionario = x.Nome, TituloCargo = x.Cargo.Titulo });

            var resultadoBuscaRapida = (IEnumerable<dynamic>)retornoBuscaRapida;

            return resultadoBuscaRapida.ToList();
        }

        public IList<dynamic> QuantidadeFuncionariosPorTurno()
        {
           var resultado = (IEnumerable<dynamic>)this.Funcionarios.GroupBy(func => func.TurnoTrabalho)
